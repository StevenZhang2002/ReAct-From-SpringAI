...
public class ContextManagementTest {
     ...
     * 场景 1: 默认上下文压缩配置。
     * <p>
     * 使用 ContextPolicy.defaults()（msgThreshold=100，tokenThreshold=90000，lastKeep=50），
     * 配合多轮工具调用任务，观察压缩效果。
     */
    ...
    static void testCustomPolicyWithProtectedTools(ChatModel chatModel) {
        ...
        ReactAgent agent = ReactAgent.builder()
                ...
                .contextPolicy(ContextPolicy.builder()
                        .msgThreshold(20)                 // 消息数达到 20 也会触发总闸门
                        .tokenThreshold(5000)             // token 总量达到 5000 也会触发压缩
                        .lastKeep(20)                     // 最近 20 条消息受保护
                        .largePayloadTokens(500)          // 内容超过 ~500 token 进入 offload 候选
                        .build())
                .maxRounds(50)
        ...
        String query = "请依次执行以下操作：1) 列出当前目录下的文件 2) 新建一个test.py，生成任意的测试python脚本，输出打印即可，必须大于50行代码 "
                + "3) 用 bash 执行 echo hello 4) 再列出当前目录文件。每一步都要调用工具。5) 在检查下当前环境python可不可用。6）在桌面新建一个1.txt，内容是hello";
        streamAndPrint(agent, query, params);
    }

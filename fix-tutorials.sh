...
fix_branch() {
    local idx=$1
    local tdir="${TUTORIALS[$idx]}"
    local num="${tdir:0:2}"
    local branch="tutorial/$tdir"
    ...
    for i in $(seq 2 $((10#$num))); do
        ...
        local src="tutorial/$src_tdir/code/spring-ai-agentx-core"
        if [ -d "$src/src/main/java" ]; then
            (cd "$src/src/main/java" && find . -name "*.java" | tar cf - -T - 2>/dev/null | tar xf - -C "$tmp_main" 2>/dev/null) || true
        fi
    done
    ...
    for i in $(seq 2 $((10#$num))); do
        ...
        [ -z "$test_tdir" ] && continue

        local test_branch="tutorial/$test_tdir"
        local test_files=$(git ls-tree -r --name-only "$test_branch" 2>/dev/null | grep "^tutorial/${test_tdir}/code/.*/src/test/" | grep "\.java$")

        if [ -n "$test_files" ]; then
            echo "$test_files" | while read f; do
                local rel="${f#tutorial/${test_tdir}/code/spring-ai-agentx-core/src/test/java/}"
    ...
    # 清理并重建
    rm -rf "tutorial/$tdir/code"
    local target="tutorial/$tdir/code"
    ...
    if [ "$main_count" -gt 0 ]; then
        local abs_main_target="$(cd "$target/spring-ai-agentx-core/src/main/java" && pwd)"
        (cd "$tmp_main" && find . -name "*.java" | tar cf - -T - 2>/dev/null | tar xf - -C "$abs_main_target" 2>/dev/null) || true
    fi
    if [ "$test_count" -gt 0 ]; then
        local abs_test_target="$(cd "$target/spring-ai-agentx-core/src/test/java" && pwd)"
        (cd "$tmp_test" && find . -name "*.java" | tar cf - -T - 2>/dev/null | tar xf - -C "$abs_test_target" 2>/dev/null) || true
    fi

...
/**
 * Agent ReAct 循环执行器 — 集成 HITL 暂停/恢复。
 * <p>
 * 本节新增：
 * <ul>
 *   <li>askUserToolName — 当工具调用匹配此名称时，拦截并返回 Paused 结果</li>
 *   <li>resumeFromMessages() — 从暂停状态恢复 ReAct 循环</li>
 * </ul>
 *
 * @author bigchui
 */
public class AgentLoopExecutor {
    ...
    public AgentLoopExecutor(ChatClient chatClient, LoopMessageBuilder messageBuilder,
        ...
        this(chatClient, messageBuilder, toolCallExecutor, maxRounds, null, null, null, null);
    }
    ...
    /**
     * 从已有消息列表恢复 ReAct 循环（用于 resume）。
     *
     * @param messages     已注入用户回答的消息列表
     * @param params       原始调用参数
     * @param startRound   起始轮次（从暂停时的轮次继续）
     * @return 执行结果
     */
    ...
    private void finishRound(List<Message> messages, Sinks.Many<AgentStreamEvent> sink,
        ...
        if (askUserToolName != null) {
            ...
            if (!pendingCalls.isEmpty()) {
                // 将 assistant 消息加入 messages（含工具调用）

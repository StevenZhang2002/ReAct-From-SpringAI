...
/**
 * Agent ReAct 循环执行器 — 集成会话持久化。
 * <p>
 * 本节新增：
 * <ul>
 *   <li>开局调用 {@code sessionPersister.initSession()} 生成 sessionId 并写开局记录</li>
 *   <li>消息构建后调用 {@code sessionPersister.snapshotMessages()} 记录快照</li>
 *   <li>终态调用 {@code sessionPersister.persistOnTerminal()} 批量落库</li>
 * </ul>
 *
 * @author bigchui
 */
public class AgentLoopExecutor {
    ...
    public AgentLoopExecutor(ChatClient chatClient, LoopMessageBuilder messageBuilder,
        ...
        this(chatClient, messageBuilder, toolCallExecutor, maxRounds, null);
    }

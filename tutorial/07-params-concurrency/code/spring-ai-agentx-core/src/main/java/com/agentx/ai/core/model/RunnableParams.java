 ...
 * Agent 调用参数。
 *
 * <p>用于传递执行时的额外参数，支持会话管理、动态参数注入和工具参数替换。
 *
 * <h3>两种参数的区别</h3>
 * <ul>
 *   <li>{@code customParams} — LLM 可见，注入到 SystemMessage 中。
 *       例：{@code addParam("language", "zh-CN")} → LLM 知道要用中文回答</li>
 *   <li>{@code toolParams} — LLM 不可见，工具执行前按 inputSchema 注入。
 *       例：{@code addToolParam("userId", "123")} → 工具收到真实 userId，不依赖 LLM 生成</li>
 * </ul>
 *
 * @author bigchui
 */

 ...
 * 工具调用执行器 — 集成 Hook 系统。
 * <p>
 * 本节新增：在工具执行前后触发 BeforeToolExecutionEvent / AfterToolExecutionEvent，
 * 允许 Hook 修改参数或检查结果。
 *
 * @author bigchui
 */
public class ToolCallExecutor {
    ...
    public ToolCallExecutor(List<ToolCallback> tools, HookManager hookManager) {
    ...
    public String execute(AssistantMessage.ToolCall toolCall, RunnableParams params) {
        ...
        // 按 inputSchema 过滤注入 toolParams
        argsJson = replaceToolParams(callback, argsJson, params);

        // 构建 ToolContext
        ToolContext toolContext = buildToolContext(params);

        // 触发 BeforeToolExecutionEvent
        BeforeToolExecutionEvent beforeEvent = new BeforeToolExecutionEvent(
                toolName, toolCallId, argsJson, toolContext);
        beforeEvent = hookManager.fireEvent(beforeEvent);
        argsJson = beforeEvent.getArguments();
        toolContext = beforeEvent.getToolContext();
        ...
        try {
            ...
            Object execResult = callback.call(argsJson, toolContext);
            result = execResult != null ? execResult.toString() : "{}";
        } catch (Exception e) {
            success = false;
        ...
        // 触发 AfterToolExecutionEvent
        AfterToolExecutionEvent afterEvent = new AfterToolExecutionEvent(
                toolName, toolCallId, argsJson, result, success, durationMs);
        hookManager.fireEvent(afterEvent);
